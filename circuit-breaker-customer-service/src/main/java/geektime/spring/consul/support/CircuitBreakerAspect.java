package geektime.spring.consul.support;
...
public class CircuitBreakerAspect {
    ...
    @Around("execution(* geektime.spring.consul.integation..*(..))")
    public Object doWithCircuitBreaker(ProceedingJoinPoint pjp) throws Throwable {
        ...
        try {
            ...
            log.warn("Circuit breaker counter: {}, Throwable: {}", counter.get(signature).incrementAndGet(), throwable.getMessage());
            breakCounter.get(signature).set(0);
            throw throwable;
        }
        return retVal;
